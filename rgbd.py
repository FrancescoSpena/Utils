...
DATASET_DIR: Final = Path("datasets/office2/results")
...
def log_office2_dataset(dataset_dir: Path, frames: int) -> None:
    """Log RGB + Depth images from the office2 dataset."""
    rr.log("world", rr.ViewCoordinates.RIGHT_HAND_Y_DOWN, static=True)

    # Prendi tutti i file depth e color
    depth_files = sorted(dataset_dir.glob("depth*.png"))
    rgb_files   = sorted(dataset_dir.glob("frame*.jpg"))

    # Limita il numero di frame se richiesto
    ...
    for idx, (rgb_path, depth_path) in enumerate(zip(rgb_files, depth_files)):
        #print(f"[DEBUG] Caricando frame {idx}: {rgb_path.name}, {depth_path.name}")
...
def main() -> None:
    ...
    log_office2_dataset(DATASET_DIR, frames=args.frames)
