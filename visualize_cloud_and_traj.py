...
def log_office2(recording_dir: Path, frames: int) -> None:
    ...
    for idx, (rgb_path, depth_path, pose) in enumerate(zip(rgb_files, depth_files, poses)):
        ...
        # --- trajectory ---
        # ricava la traslazione dalla Transform3D di rerun
        # print(type(pose.translation))
        # exit(0)
        ...
        # inverti per ottenere w2c
        w2c = np.linalg.inv(c2w)
        R, t = w2c[:3, :3], w2c[:3, 3]

        # centro camera in world frame
        cam_center = (-R.T @ t).astype(np.float32)
        traj_points.append(cam_center)

        # logga i punti della traiettoria
        rr.log("world/trajectory/points",
        rr.Points3D(positions=np.vstack(traj_points),
                    radii=0.01,
                    colors=[0, 0, 255]))  # rosso

        rr.log("world/trajectory/lines",
            rr.LineStrips3D([np.vstack(traj_points)],
                            radii=0.01,
                            colors=[0, 0, 255]))  # verde

        if points_world is not None:
            #vals = [float(x) for x in open(traj_file).readlines()[idx].split()]
            w2c = np.array(vals, dtype=np.float32).reshape(4, 4)
            w2c[:3, 1] *= -1
            w2c[:3, 2] *= -1


            new_idx, new_pts, new_cols = visible_points(points_world, colors, w2c,
                                                        INTRINSIC, RESOLUTION[0], RESOLUTION[1])

            seen_indices.update(new_idx)  # accumula

            rr.log("world/progressive_pointcloud",
                rr.Points3D(positions=points_world[list(seen_indices)],
                            colors=colors[list(seen_indices)]))
